...
exports.canUSB = function (USB_PORT, NET_PORT, NET_ADDRESS) {
    ...
    client.connect(NET_PORT, NET_ADDRESS, function () {
        ...
        console.log(`canUSB4 : Client Connected to ${USB_PORT}`)
    })
    ...
    client.on('data', function (data) {
        ...
        for (let i = 0; i < outMsg.length - 1; i++) {
            ...
            if (message) {
                ...
                console.log(`${USB_PORT} -> CbusServer Message Received : ${message}`)
            }
        }
    })
    ...
    serialPort.on("open", function () {
        ...
        console.log(`canUSB4 Serial Port : ${USB_PORT} Open`)
    })
    
    parser.on('data', function (data) {
        //winston.info({message: `${USB_PORT} -> Message Parsed : ${message}`})
        //console.log(`${USB_PORT} -> Message Parsed : ${data}`)
        let message = getValidMessage(data);    // rebuild message as string
        if (message) {
            //winston.info({message: `${USB_PORT} -> Message Parsed : ${message}`})
            //console.log(`${USB_PORT} -> Message Parsed : ${message}`)
...
function getValidMessage(data) {
    ...
    if (array.length == 1) {
        ...
        console.log(`message rejected - missing starting character: ${data.toString()}`)
        return undefined;
    }
    ...
    if (array.length == 2) {
        if (array[0].length > 0) {
            ...
            console.log(`unexpected characters in message: ${data.toString()}`)
        }
    }
    ...
    if (array.length > 2) {
        ...
        console.log(`multiple starting characters in message: ${data.toString()}`)
    }
    ...
    if ( (message[1] != 'S') && (message[1] != 'X') ) {
        ...
        console.log(`message rejected - unknown Identifier type: ${message}`)
        return undefined;
    }
    ...
    if (splitMessage.length == 1) {
        ...
        console.log(`message rejected - unknown Transmission type: ${message}`)
        return undefined;
    }
    ...
    if (splitMessage.length > 2) {
        ...
        console.log(`message rejected - unexpected N or R characters in message: ${message}`)
        return undefined;
    }
    ...
    if (message.includes('S')) {
        ...
        if ( splitMessage[0].length != 6 ) {
            ...
            console.log(`message rejected - Standard Identifier field wrong length: ${message}`)
            return undefined;
        }
    }
    if (message.includes('X')) {
        ...
        if ( splitMessage[0].length != 10 ) {
            ...
            console.log(`message rejected - Extended Identifier field wrong length: ${message}`)
            return undefined;
        }
    }
    ...
    if ( splitMessage[1].length > 17 ) {
        ...
        console.log(`message rejected - Data field too long: ${message}`)
        return undefined;
    }
